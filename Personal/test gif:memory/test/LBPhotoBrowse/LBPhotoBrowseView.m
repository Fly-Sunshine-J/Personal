...
@interface LBPhotoBrowseView ()<UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout>
...
@implementation LBPhotoBrowseView
...
- (UIPageControl *)pageControl {
    if (!_pageControl) {
        ...
        pageControl.currentPage = [LBPhotoBrowseManager defaultManager].selectedIndex;
...
- (UICollectionView *)collectionView {
    if (!_collectionView) {
        ...
        collectionView.bounces = [LBPhotoBrowseManager defaultManager].isNeedBounces;
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        [LBPhotoBrowseManager defaultManager].currentCollectionView = self.collectionView;
    }
    return self;
}
...
- (void)collectionView:(UICollectionView *)collectionView willDisplayCell:(UICollectionViewCell *)cell forItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    LBPhotoBrowseManager *mgr = [LBPhotoBrowseManager defaultManager];
    ...
    if (image) {
       ...
       // 1 低内存 -- > 当前的Image
        ...
        [[SDWebImageManager sharedManager] loadImageWithURL:self.urls[indexPath.row] options:0 progress:^(NSInteger receivedSize, NSInteger expectedSize, NSURL * _Nullable targetURL) {
            LBPhotoBrowseLog(@"%d -- %d",(int)receivedSize ,(int)expectedSize);
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if (!LBPhotoBrowseManager.defaultManager.currentShowImageView) {
        return;
    }
    ...
    LBPhotoBrowseManager.defaultManager.currentShowImageView = [cell.zoomScrollView valueForKeyPath:@"imageView"];
}
...
- (void)changeImageViewsHideStatus {
    ...
    UIImageView *currentImageView = [LBPhotoBrowseManager defaultManager].imageViews[selectedIndex];
