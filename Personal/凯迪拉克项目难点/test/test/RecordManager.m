...
- (id)init {
    ...
    if (self) {
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(interruptHandle:) name:AVAudioSessionInterruptionNotification object:[AVAudioSession sharedInstance]];
...
//            CLog(@"err----%@", err.localizedDescription);
    ...
    else if (inInterruptionState == kAudioSessionEndInterruption)
    {
        if (_started) {
//            NSError *err, *err1;
//            [[AVAudioSession sharedInstance] setCategory:AVAudioSessionCategoryPlayAndRecord error:&err1];
