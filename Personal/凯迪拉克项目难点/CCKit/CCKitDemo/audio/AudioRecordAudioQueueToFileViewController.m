...
- (void)initView {
    ...
    _basicDescription.mSampleRate = 8000;// 44.1HZ
...
- (void)createAudioQueueRecorderAndAudioFile {
    ...
    NSString *fileName = [NSString stringWithFormat:@"%ld.m4a", (long)[current timeIntervalSince1970]];
    ...
    OSStatus status = AudioFileCreateWithURL(audioFileURL, kAudioFileM4AType, &_basicDescription, kAudioFileFlags_EraseFile, &obj->_audioFileID);
    if (status != noErr) {
